...
ggplot() +
  ...
  geom_sf(data = huc8_sf_2, color = 'blue', fill = 'yellow')
...
wind_energy_raw <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses = c('HUC_12' = 'factor')) 

wind_energy_wrangled <- wind_energy_raw %>% 
  mutate(HUC_8 = substr(wind_energy_raw$HUC_12, 1, 8)) %>% 
  group_by(HUC_8) %>% 
  summarize(Sum_Wind_Energy = sum(AvgWindEnergy))
 
#Join to HUC_8 features

wind_energy_join <- left_join(huc8_sf, wind_energy_wrangled)

#View the outputs
ggplot(data=wind_energy_join) +
...
myMap2 + mapview(triCo_HUC8_sf, alpha.regions = 0, color='red')
```
