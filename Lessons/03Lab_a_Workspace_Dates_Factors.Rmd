...
files <- dir(filefolder, pattern = "EPAair_O3*.*", full.names = TRUE) #dot and asterix indicating that what's after is different
EPAData <- files %>% #pipe structure
  map(read_csv) %>%
  bind_rows() #telling R to bind the files by row
...
#Convert to a factor
sites$dow <- factor(sites$dow) # because numbers are not showing/leading to any new/useful info
