...
pm25.ozone.regression <-
  lm(data = EPAair, PM2.5 ~ Ozone)
summary(pm25.ozone.regression)

#r2 is 13%, so there are more things driving the variability of the response
...
EPAairCorr <- cor(EPAairmeasures)
corrplot(EPAairCorr, method = "ellipse")
corrplot.mixed(EPAairCorr, upper = "ellipse")

# Mean_Lat and month next best variables to add...
...
airAIC <- lm(data = EPAairmeasures, PM2.5~Ozone + Year + Month + mean_Lng + mean_Lat)

step(airAIC)

#looking for variables that you remove and result in lower AIC
...
#add in all the relevant variables to create the best possible regression

best.regression <- lm(data = EPAairmeasures, PM2.5~Ozone + mean_Lng + Month + mean_Lat)
