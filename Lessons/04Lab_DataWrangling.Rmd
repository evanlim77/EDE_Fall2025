...
#Install packages
library(tidyverse) # a meta package that includes a host of other packages
library(lubridate)
...
# Supply the data frame and the filter statement
...
# dplyr filtering 
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0) # can use equivalence since depth is numeric

# Choose multiple conditions to filter
summary(NTL.phys.data$lakename)
NTL.phys.data.PeterPaul <- 
  filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) # could use equals... but the in clause helps if you'd have a lot of names

# Choose a range of conditions of a numeric or integer variable
summary(NTL.phys.data$daynum)
NTL.phys.data.JunethruOctober <- filter(NTL.phys.data, daynum %in% c(152:304)) # filtering where day number is between 152 and 304
```
...
NTL.phys.data.1999 <- filter(NTL.phys.data, year4 == 1999) # Filtering the data by the year 1999, using the column "year4"
# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.Tuesday90to992 <- 
  filter(NTL.phys.data, lakename == "Tuesday Lake", year4 %in% 1990:1999)

NTL.phys.data.Tuesday90to99 <- NTL.phys.data %>%
  filter(
    lakename == "Tuesday Lake", # could use | for "or" or & for "and"
    year4 %in% 1990:1999
  )
...
> Answer: We don't control those numbers! Can change as things shift
...
NTL.phys.data.processed <- NTL.phys.data %>% 
  filter(
    lakename == "Paul Lake" | lakename == "Peter Lake") %>%  
  select(
    lakename, sampledate:temperature_C) %>% 
  mutate(
    temperature_F = (temperature_C*9/5) + 32)

#Exercise 3: Using a pipe filter NTL.phys.data for Tuesday Lake from 1990 
# through 1999 only for July. should get 537 rows
ex3 <- NTL.phys.data %>% 
  filter(
    lakename == "Tuesday Lake", year4 %in% 1990:1999) %>% 
  mutate(
...
# Need list of column names to squeeze together, specify where the columns will go, and new column names

NTL.phys.data.gathered <- pivot_longer(
  NTL.phys.data, irradianceWater:irradianceDeck, names_to = "Irradiance_Type", values_to = "Irradiance_Value"
)

#Exercise 6: Spread temperatureC into more than one column based on the depth.
NTL.phys.data.spread <- pivot_wider(
  NTL.phys.data, names_from = depth, values_from = temperature_C
)
