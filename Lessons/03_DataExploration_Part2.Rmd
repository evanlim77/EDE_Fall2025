...
>Answer: Understanding the dimensions, the way data was collected... the current format and potential things you may need to update... Exploring the data ensures you have a strong understanding of it before you begin to manipulate it and draw conclusions from it. Drawing conclusions from a data set that is not entirely clean is tricky as the resulting numbers/info may not be entirely accurate.
...
ggplot(USGS.flow.data) +
  geom_point(aes(x = datetime, y = discharge.max))
```
...
> Answer: It's interesting to see all the various kinds of graphs. A lot of the values seem to max out at the lower end of things. It does seem there is something of a positive correlation between discharge mean and gage height mean. It's also important to remember that the "mean" values are higher than the "max" values on a histogram because that's actually measuring how many times a given value is appearing and not what the actual value is.


How can multiple options for data exploration inform our understanding of our data?

> Answer: One thing that is particularly useful in this sense is changing the bin width on a histogram for example. It's similar to viewing it at a higher resolution. Being able to manipulate large data sets on command like that is quite powerful.
...
> Answer: It looks like gage height minimum data only started being collected a bit before 2000. And there is that positive correlation between discharge mean and gage height mean. I suppose it makes sense for if the gage height goes up, meaning the level of water goes up, then there should be a higher volume of water flowing through any given point.  
