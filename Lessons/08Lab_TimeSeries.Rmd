...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% #indicating the separator, which is a space in this instance, and then naming the three columns as there is a space before the month that we also need to account for
  mutate( Date = my(paste0(Month,"-",Year))) %>% # creating a new column combining month and year separated by a dash, also converting character to a date with my function
...
#seasonality indicated by up and down movements that are regularly spaced
...
BR_Rain_ts <- ts(BR_complete$Rainfall, #make sure you point to column you want to convert to a time series
...
BR_Temp_decomp <- stl(BR_Temp_ts,s.window = "periodic") #linear trend indicated that temp started increasing earlier than it actually did... shows that stl is a better representation of the trend than the linear regression
plot(BR_Temp_decomp)
...
BR_Rain_decomp_dataframe <- as.data.frame(BR_Rain_decomp$time.series)

BR_Temp_decomp_dataframe <- as.data.frame(BR_Temp_decomp$time.series)

BR_Rain_decomp_dataframe <- mutate(BR_Rain_decomp_dataframe,
                                   Observed = BR_complete$Rainfall,
                                   Date = BR_complete$Date)

BR_Temp_decomp_dataframe <- mutate(BR_Temp_decomp_dataframe,
                                   Observed = BR_complete$Temperature_C,
                                   Date = BR_complete$Date)

# Exercise 10: Visualize how the trend maps onto the data for both series

ggplot(BR_Rain_decomp_dataframe)+
  geom_line(aes(y=Observed,x=Date), size = 0.25)+
  geom_line(aes(y=trend,x=Date))

# Keep in mind this is country wide data, so regional trends may differ be more dramatic
