...
# Comparing value to specific value
`==` 
# Seeing if values are different 
`!=`
...
`>=`
# Aggregate multiple conditions and want both to be true
`&`
# Multiple conditions with one or the other being true
`|`
...
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) # Saves from having to write lakename == over and over again, if you're looking to include many
