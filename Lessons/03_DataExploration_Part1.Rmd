...
> Answer: The same line of code just appears. I don't see any other changes
...
> Answer: They were likely given the value NA as all the vectors in a data frame need to be the same length
...
> Answer: You likely would want to remove NAs when calculating anything quantitative as they will disrupt the functions/calculations. When trying to see broader trends across a few variables, it might be helpful to have the NAs? so that not too many options are eliminated for having just a single one.
...
```{r Fixing dates}
# This is changing the format of the dates from month, date, year to year, month, date
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d")

# This is creating a new function... where if d is greater than 191226, then you use 19 and if not then you use 20...
...
# And then setting it as a date again changing the format where the year is 4 digits
